...
class SSAM(nn.Module):
    """
        Feature Mutual Alignmen Module
        """
    ...
    def compute_distances(self, query_a, key_a, value_a, query_b, key_b, value_b, features_a, features_b):
        ...
        aligned_features_b = torch.matmul(att_probs, value_a)
        ...
        aligned_features_a = torch.matmul(att_probs, value_b)
        ...
        sq_similarity = -torch.sum((value_a.view(value_a.size(0), value_a.size(1), -1) - aligned_features_a.view(
            aligned_features_a.size(0), aligned_features_a.size(1), -1)) ** 2, dim=-1)
        qs_similarity = -torch.sum((value_b.view(value_b.size(0), value_b.size(1), -1) - aligned_features_b.view(
            aligned_features_b.size(0), aligned_features_b.size(1), -1)) ** 2, dim=-1)
